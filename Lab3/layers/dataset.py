...
def cifar100(seed, c=5):
    ...
    with open(CIFAR_PATH + 'train', 'rb') as fo:
        # train_data = cPickle.load(fo, encoding='latin1')
    ...
    with open(CIFAR_PATH + 'test', 'rb') as fo:
        # test_data = cPickle.load(fo, encoding='latin1')
    ...
    # select categories
    selected_cats = np.random.choice(np.arange(0, 100), c, replace=False)
    ...
    return (train_x, train_y), (test_x, test_y), selected_cats
