...
class TestImageClass(unittest.TestCase):
    ...
    def test_transpose(self):
        """
        Test ImageClass.rotate90 function
        """
        # Test with some random noise, because sparky has too many
        # same valued pixels
        np.random.seed(1337)
        ...
        # copy the original and perform rotation
        orig_im = np.copy(self.im_object.im)
        self.im_object.transpose()

        # check some pixels that should be rotated
        self.assertTrue(np.array_equal(self.im_object.im[5, 10, :],
                                       orig_im[10, 5, :]))

        # check some pixels that should not be rotated
    ...
    def test_blur(self):
        ...
        # check that we are computing the average of 7x7 regions
        # allow a tolerance of 1 grayscale value
        # we check the three channels separately
        off_r = 60
        off_c = 120
        self.assertTrue(np.mean(orig_im[off_r:off_r+7, off_c:off_c+7, 0]) -
                        self.im_object.im[off_r, off_c, 0] <= 1)
        self.assertTrue(np.mean(orig_im[off_r:off_r+7, off_c:off_c+7, 1]) -
                        self.im_object.im[off_r, off_c, 1] <= 1)
        self.assertTrue(np.mean(orig_im[off_r:off_r+7, off_c:off_c+7, 2]) -
